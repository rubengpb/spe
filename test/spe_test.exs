defmodule SPETest do
  ...
  test "submit_bad_jobs" do
    ...
    assert {:error,_} = SPE.submit_job(%{"name" => "nisse", "tasks" => [task]})
    task = %{"name" => "t0", "enables" => ["t1"], "exec" => fn _ -> 1+2 end, "timeout" => :infinity}
    assert {:error,_} = SPE.submit_job(%{"name" => "nisse", "tasks" => [task]})
    task = %{"name" => "t0", "enables" => [], "exec" => fn _ -> 1+2 end, "timeout" => :infinity}
  ...
  test "start_job3" do
    ...
    SPE.start_job(id)
    all_broadcasts = get_all_broadcasts(300)
    assert {:succeeded, %{ "t0" => {:result,1}, "t1" => {:result,2} }} = get_result(id, all_broadcasts)
  end
  ...
  test "start_job5" do
    ...
    SPE.start_job(id)
    all_broadcasts = get_all_broadcasts(300)
    assert {:succeeded, %{ "t0" => {:result,3}, "t1" => {:result,7} }} = get_result(id, all_broadcasts)
  end
  ...
  test "start_job_failing" do
    ...
    task3 = %{"name" => "t3", "enables" => [], "exec" => fn(_) -> throw "dont catch me" end, "timeout" => :infinity}
    result = SPE.submit_job(%{"name" => "nisse", "tasks" => [task0,task1,task2,task3]})
    assert {:ok, id} = result
    Phoenix.PubSub.subscribe(SPE.PubSub,id)
    SPE.start_job(id)
    all_broadcasts = get_all_broadcasts(300)
    assert {:failed, %{ "t0" => {:failed,_}, "t1" => {:failed,_}, "t2" => {:failed,_}, "t3" => {:failed,_} }} = get_result(id, all_broadcasts)
  end
  ...
  test "job_enables2" do
    ...
    task2 = %{"name" => "t1", "enables" => ["t0"], "exec" => fn(_) -> 3+4 end, "timeout" => :infinity}
    task3 = %{"name" => "t2", "enables" => ["t0"], "exec" => fn(_) -> 8+9 end, "timeout" => :infinity}
    result = SPE.submit_job(%{"name" => "nisse", "tasks" => [task1,task2,task3]})
    assert {:ok, id} = result
    Phoenix.PubSub.subscribe(SPE.PubSub,id)
    SPE.start_job(id)
    all_broadcasts = get_all_broadcasts(500)
    assert {:succeeded, %{ "t0" => {:result,26}, "t1" => {:result,7}, "t2" => {:result,17} }} = get_result(id, all_broadcasts)
  end
  ...
  test "job_enables_transitive" do
    ...
    SPE.start_job(id)
    all_broadcasts = get_all_broadcasts(500)
    assert {:succeeded, %{ "t2" => {:result,17}, "t1" => {:result,21}, "t0" => {:result,40} }} = get_result(id, all_broadcasts)
  end
  
  test "timeout_job1" do
    assert {:ok,_} = SPE.start_link([])
    task0 = %{"name" => "t0", "enables" => ["t1"], "exec" => fn _ ->
              Process.sleep(1000)
              2
            end, "timeout" => 200}
  ...
  test "big_test" do
    assert {:ok,_} = SPE.start_link([])
    limit = 100
    tasks = for i <- 1..limit do
      %{"name" => "t_"<>Integer.to_string(i), "enables" => [], "exec" => fn(_) -> i end, "timeout" => :infinity}
    end
  ...
  test "big_job_test" do
    assert {:ok,_} = SPE.start_link([])
    limit = 100
    jobs = for _ <- 1..limit do
      task1 = %{"name" => "t0", "enables" => [], "exec" => fn(%{"t1" => value}) -> value+2 end, "timeout" => :infinity}
      task2 = %{"name" => "t1", "enables" => ["t0"], "exec" => fn(_) -> 3+4 end, "timeout" => :infinity}
      %{"name" => "stina", "tasks" => [task1, task2]}
    end

    job_ids = Enum.map(jobs, fn job ->
  ...
  test "multiple_jobs" do
    ...
    SPE.start_job(id2)
    all_broadcasts = get_all_broadcasts(1000)
    assert {:succeeded, %{ "t0" => {:result,9}, "t1" => {:result,7} }} = get_result(id1, all_broadcasts)
    assert {:succeeded, %{ "t0" => {:result,9}, "t1" => {:result,7} }} = get_result(id2, all_broadcasts)
  end
  ...
  test "task_timing1" do
    ...
    task2 = %{"name" => "t1", "enables" => [], "exec" => fn(_) ->
               Process.sleep(600)
               3+4
             end, "timeout" => :infinity}
  ...
  def task_start_time(all_broadcasts, job_id, task_id) do
    assert [time] = Enum.reduce(all_broadcasts, [], fn broadcast, acc ->
      case broadcast do
  ...
  def get_result(id, all_broadcasts) do
    assert [{^id,:result,result}] = Enum.filter(get_all_results(all_broadcasts), fn result ->
      case result do
        {^id,:result,_} -> true
        _ -> false
      end
  ...
  def get_all_broadcasts(timeout, broadcasts) do
    ...
    after timeout -> Enum.reverse(broadcasts)
    end
  end
